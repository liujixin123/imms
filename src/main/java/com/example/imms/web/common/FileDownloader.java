...
package com.example.imms.web.common;
...
public class FileDownloader {
     ...
     */
    private final static boolean isRequestComeFromWidnows(HttpServletRequest request) {
        String agent = request.getHeader("user-agent");
        if (StringUtils.isNotEmpty(agent)) {
            return agent.toLowerCase().indexOf("windows") >= 0;
        } else {
            return false;
        }
    }
            ...
            if (preIndex != -1) {
                ...
                if (length > 0) {
                    ...
                    if (sepIndex >= 0) {
                        ...
                        int begin = NumberUtils.toInt(str, -1);
                        ...
                        if (sepIndex < length - 1) {
                            ...
                            int end = NumberUtils.toInt(str, -1);

                            if (end != -1)
                                range.setEnd((begin != -1) ? end : -end);
                        }
                    } else {
                        int point = NumberUtils.toInt(header, -1);
    ...
    private final static String getRequestRealPath(HttpServletRequest request, String path) {
