...
import com.example.imms.web.dto.LoginDTO;
...
//            data.put("url",request.getContextPath()+"/home");
    ...
    @RequestMapping("/userManageInner")
    public String userManageInner() {
        return "user/userManageInner";
    }
    ...
    @RequestMapping(value = "/getUserMsgSignList", method = RequestMethod.POST)
    @ResponseBody
    public PageDataResult getUserMsgSignList(@RequestParam("pageNum") Integer pageNum,
                                      @RequestParam("pageSize") Integer pageSize,/*@Valid PageRequest page,*/ UserSearchDTO userSearch
                                             ) {
        /*logger.info("分页查询用户列表！搜索条件：userSearch：" + userSearch + ",pageNum:" + page.getPageNum()
                + ",每页记录数量pageSize:" + page.getPageSize());*/
        ...
        try {
            ...
            // 获取用户列表
            pdr = adminUserService.getUserMsgSignList(userSearch, pageNum ,pageSize);
            logger.info("用户列表查询=pdr:" + pdr);
